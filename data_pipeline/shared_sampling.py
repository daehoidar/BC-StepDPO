...
def main():
    ...
    with open(out_path, "a", encoding="utf-8") as fout:
        for i, prob in enumerate(rows):
            ...
            # 한 행(problem×persona)의 모든 (sample, step) 검증을 병렬 실행.
            # step은 prefix가 미리 정해져 서로 독립적이고 verifier._log은 락으로
            # 보호되므로 동시 호출이 안전하다. openai_client._EXEC(16워커) 고갈을
            # 피해 워커는 8로 둔다(순차 ~36 calls/min → 약 8배). 한 step의 검증
            # 실패가 행 전체를 죽이지 않도록 fallback 라벨로 흡수(무인 실행 견고성).
            def _verify_label(step, prefix):
                try:
                    ...
                    return {
                        ...
                        "trigger_term": None, "evidence_code": None,
                        "first_introduced": None,
                        "reasoning": f"verify failed: {e}",
                    }

            tasks = []  # (offset, j, step, prefix)
            for offset, steps in enumerate(chains):
                if len(steps) < 2:
                    continue
                for j, step in enumerate(steps):
                    tasks.append((offset, j, step, steps[:j]))

            labels_by_offset: dict[int, dict[int, dict]] = defaultdict(dict)
            if tasks:
                with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
                    futs = {ex.submit(_verify_label, t[2], t[3]): (t[0], t[1])
                            for t in tasks}
                    for fut in concurrent.futures.as_completed(futs):
                        off, j = futs[fut]
                        labels_by_offset[off][j] = fut.result()
            ...
            for offset, steps in enumerate(chains):
                ...
                if len(steps) < 2:
                    ...
                    per_step = labels_by_offset[offset]
                    step_labels = [per_step[j] for j in range(len(steps))]
                ...
                n_new += 1
            fout.flush()  # 행 단위로 디스크에 반영 (무인 실행 진행 가시성·손실 최소화)
